...
export class ArenaScene extends Phaser.Scene {
  ...
  create(): void {
    // M11 — сессия встречи: seed, активная вкладка, слепой источник и выборы
    // переживают scene.restart() (переключение вкладок не пересоздаёт задание).
    const sess = gameState.getArena();
    let tpl: ReturnType<ArenaScene['pickTemplate']>;
    if (sess) {
      tpl = templateFor(sess.enemyId, sess.stage);
      this.encounter = mutate(tpl, sess.seed);
      this.activeSource = (sess.activeSource as SourceId | null) ?? (this.encounter.sources[0] as SourceId);
      this.blindOpened = sess.blindOpened;
      this.selectedEvidence = new Set(sess.selectedEvidence);
      this.selectedAnswer = sess.selectedAnswer;
      this.selectedSequence = [...sess.selectedSequence];
      this.verdictFactor = sess.verdictFactor;
      this.confidence = sess.confidence;
    } else {
      ...
      tpl = this.pickTemplate();
      const s = gameState.startArena(tpl.enemyId, tpl.stage);
  ...
  private createBrowser(): void {
    ...
    tabs.forEach((sid,i)=>{
      ...
      this.add.rectangle(tx, by+22, tabW, 28, bg).setStrokeStyle(1, border).setOrigin(0).setInteractive().on('pointerdown', ()=>{
        if(isBlind){
          // M9 — цена меньше цены ошибки (защита); списываем один раз за встречу
          ...
          gameState.patchArena({ blindOpened:true, activeSource: sid });
          this.scene.restart();
          return;
        }
        this.activeSource=sid;
        gameState.patchArena({ activeSource: sid });
        this.scene.restart();
      });
    ...
    if(isVerdict && !this.verdictFactor){
      ...
      opts.forEach((o,i)=>{
        ...
        this.add.rectangle(14,ay,362,36, this.COLORS.surface).setStrokeStyle(1, this.COLORS.border).setOrigin(0).setInteractive().on('pointerdown', ()=>{
          this.verdictFactor=o.k as any; gameState.patchArena({ verdictFactor: this.verdictFactor }); this.scene.restart();
        });
    ...
    answers.forEach((a,i)=>{
      ...
      this.add.rectangle(ax,ay,178,60, isSel? C.accentN : C.surface, isSel?1:0.94).setStrokeStyle(1, isSel? C.accentN : C.border).setOrigin(0).setInteractive().on('pointerdown', ()=>{
        ...
        if(tilt && !this.selectedAnswer){
          ...
          this.time.delayedCall(balanceConfig.coldHead.delayMs, ()=> { this.selectedAnswer=i; gameState.patchArena({ selectedAnswer: i }); this.showConfidencePicker(); });
          return;
        }
        this.selectedAnswer=i; gameState.patchArena({ selectedAnswer: i }); this.showConfidencePicker();
      });
  ...
  private showConfidencePicker(){
    ...
    opts.forEach((o,i)=>{
      ...
      this.add.rectangle(x,by+20,108,28, this.COLORS.surface).setStrokeStyle(1, this.COLORS.border).setOrigin(0).setInteractive().on('pointerdown', ()=>{
        this.confidence=o.k; gameState.patchArena({ confidence: o.k }); this.submitAnswer();
      });
  ...
  private showConfidenceAfter(cb:()=>void){
    ...
    opts.forEach((o,i)=>{
      const x=20+i*114;
      this.add.rectangle(x,by+20,108,28, this.COLORS.surface).setStrokeStyle(1, this.COLORS.border).setOrigin(0).setInteractive().on('pointerdown', ()=>{ this.confidence=o.k; gameState.patchArena({ confidence: o.k }); cb(); });
