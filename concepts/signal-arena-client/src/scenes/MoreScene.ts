...
export class MoreScene extends Phaser.Scene {
  ...
  private openSheet(kind:string){
    ...
    const calLine = cal.length>2
      ? `калибровка M3: прогноз ${(cal.reduce((a,b)=>a+b.predicted,0)/cal.length).toFixed(2)} · факт ${(cal.reduce((a,b)=>a+b.actual,0)/cal.length).toFixed(2)}`
      : 'калибровка M3: данных мало — отвечай со ставкой';
    const rows = [
      `УРОВЕНЬ ${p.level} · ${ep.name}`,
    ...
    this.add.text(195, 520, '✕ закрыть', { fontFamily:'Inter, sans-serif', fontSize:'12px', color:'#93A3BC' }).setOrigin(0.5).setInteractive().on('pointerdown', ()=> destroyFrom(this, sheet));
  }
}
