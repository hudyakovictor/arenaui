  ...
  create(): void {
    ...
    toShow.forEach((e,i)=>{
      ...
      this.add.rectangle(cx,cy,86,86, 0x0C1323).setStrokeStyle(1, col).setOrigin(0).setInteractive().on('pointerdown', ()=>{
        ...
        this.add.text(195, 606,'✕ закрыть', { fontFamily:'Inter, sans-serif', fontSize:'12px', color:'#93A3BC'}).setOrigin(0.5).setInteractive().on('pointerdown', ()=> destroyFrom(this, sheet));
      });
