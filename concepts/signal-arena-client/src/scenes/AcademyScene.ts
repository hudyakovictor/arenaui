...
export class AcademyScene extends Phaser.Scene {
  ...
  private showLesson(card: typeof cards[number]){
      ...
      r.on('pointerdown', ()=>{
        ...
        if(o.ok){
          ...
          this.time.delayedCall(900, ()=>{
            destroyFrom(this, overlay); // сносим урок целиком: фон + тексты + кнопки
    ...
    this.add.text(195, 720, 'закрыть ✕', { fontFamily:'Inter, sans-serif', fontSize:'12px', color:'#62708A'}).setOrigin(0.5).setInteractive().on('pointerdown', ()=> destroyFrom(this, overlay));
  }
  ...
  private createBottomNav(){
    ...
    const items=[
      ...
      {label:'COLLECTION', active:false, key:'CollectionScene'},
      {label:'MORE', active:false, key:'MoreScene'},
    ] as any[];
    items.forEach((it,i)=>{
      ...
      if(!isUnlocked) this.add.text(nx+390/8+34,806,'🔒', { fontSize:'7px'}).setOrigin(0.5);
    });
  }
}
