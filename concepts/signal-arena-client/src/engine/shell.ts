...
export function destroyFrom(scene: Phaser.Scene, obj: Phaser.GameObjects.GameObject): void {
  const list = scene.children.list;
  const idx = (list as Phaser.GameObjects.GameObject[]).indexOf(obj);
  if (idx < 0) { obj.destroy(); return; }
