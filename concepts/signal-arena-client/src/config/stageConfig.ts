import type { StageId, SourceId } from '../types';
...
export interface StageDef {
  ...
  motto: string;
  description: string;
  // токены оформления — разница стадий при одном скелете
  tokens: {
...
export function stageOf(level: number): StageDef { return stages[getStageForLevel(level)]; }

export const mechanicsIntro: Record<string, {stage: StageId, level: number, note: string}> = {
  M1_Evidence: { stage: 'street', level: 1, note: 'улики + evidence strip' },
  M6_PlayForward: { stage: 'street', level: 1, note: 'график доигрывает' },
  M10_ColdHead: { stage: 'street', level: 1, note: 'вариант ЖДАТЬ всегда в пуле' },
  M11_Mutation: { stage: 'street', level: 1, note: 'детерминированная мутация' },
  M12_Campaign: { stage: 'street', level: 1, note: 'враг 3-4 стадии' },
  M13_Weather: { stage: 'street', level: 1, note: 'погода дня' },
  M15_Budget: { stage: 'street', level: 1, note: 'бюджет риска в топ-баре' },
  M3_Confidence: { stage: 'street', level: 3, note: 'ставка низко/средне/высоко' },
  M5_Identify: { stage: 'street', level: 8, note: 'опознание врага после ответа' },
  M2_Sequence: { stage: 'street', level: 14, note: 'стек 2 карты' },
  M4_Verdict: { stage: 'cabinet', level: 21, note: 'вердикт конфликта' },
  M7_Scroll: { stage: 'cabinet', level: 21, note: 'свиток ошибок полный' },
  M8_Combo: { stage: 'cabinet', level: 28, note: 'двойки' },
  M9_Blind: { stage: 'cabinet', level: 30, note: 'слепой источник' },
};
