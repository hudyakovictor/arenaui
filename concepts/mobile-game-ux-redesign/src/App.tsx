...
import { EPOCHS, EPOCH_ORDER, epochForLevel, tokensToCss, type EpochDef, type NavId } from './theme/epochs';
...
import { BottomNav, EpochTransition, StatsSheet, type ResultData } from './components/Overlays';
...
export default function App() {
  ...
  const [transition, setTransition] = useState<{ from: EpochDef; to: EpochDef } | null>(null);

  const epoch = useMemo(() => epochForLevel(player.level), [player.level]);
  ...
  const onNext = () => {
    // демо: каждая встреча = +3 уровня, чтобы смена эпох была наблюдаема
    const nextLevel = Math.min(99, player.level + 3);
    const from = epoch;
    const to = epochForLevel(nextLevel);
  ...
  const texture = epoch.tokens.texture;
  ...
  return (
    <div className="min-h-screen w-full bg-[#050506] text-white lg:flex lg:items-start lg:justify-center lg:gap-10 lg:px-8 lg:py-8">
      ...
      <div
        ...
        style={{ ...tokensToCss(epoch.tokens), background: 'var(--bg)', color: 'var(--text)', fontFamily: 'var(--font-ui)' }}
      >
        ...
        <main className="relative z-10 flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
          <div key={`${epoch.id}-${nav}`} className="animate-[fadein_.25s_ease-out] min-h-full">
            {nav === 'arena' && <ArenaScreen epoch={epoch} player={player} round={round} onOpenStats={() => setStats(true)} onResult={onResult} onNext={onNext} />}
            {nav === 'academy' && <AcademyScreen epoch={epoch} onGoArena={() => setNav('arena')} />}
            {nav === 'journal' && <JournalScreen epoch={epoch} onGoArena={() => setNav('arena')} />}
            {nav === 'collection' && <CollectionScreen epoch={epoch} />}
            {nav === 'more' && <MoreScreen epoch={epoch} player={player} />}
          </div>
        </main>

        <BottomNav epoch={epoch} active={nav} onNav={setNav} />

        <StatsSheet open={stats} onClose={() => setStats(false)} epoch={epoch} level={player.level} xp={player.xp} sig={player.sig} streak={player.streak} weather="Тренд" budget={player.budget} onSetLevel={setLevel} />

        {transition && <EpochTransition from={transition.from} to={transition.to} onContinue={() => setTransition(null)} />}
      </div>

      {/* СПЕЦИФИКАЦИЯ — только на широких экранах */}
      <aside className="hidden w-[420px] shrink-0 lg:block">
        <SpecPanel epoch={epoch} onPick={setLevel} />
      </aside>
    </div>
  );
}

function SpecPanel({ epoch, onPick }: { epoch: EpochDef; onPick: (l: number) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-[13px] text-zinc-300">
      ...
      <p className="mt-2 text-zinc-400">Эпоха меняет структуру обучения, а не только палитру. Терминал — финальная форма: к виду биржи игрок приходит последним.</p>
      ...
      <div className="mt-4 text-[11px] uppercase tracking-wider text-zinc-500">Контракт структуры · {epoch.name}</div>
      ...
      <ol className="mt-2 space-y-1 text-zinc-400">
        <li>1. Инфо-строка: уровень/эпоха · бюджет риска</li>
        <li>2. Задача: одна строка вопроса (+ шаг в эпохе I)</li>
        ...
        <li>8. Навигация — тихая, растёт по эпохам</li>
      </ol>
      <p className="mt-4 text-[11px] text-zinc-500">Для демонстрации смены эпох: каждая встреча = +3 уровня, или перемотка в листе статистики (иконка справа сверху в телефоне).</p>
    </div>
  );
}
