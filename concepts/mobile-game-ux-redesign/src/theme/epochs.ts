// SIGNAL ARENA — контракт эпох v2.
// Принцип: один скелет экрана, но эпоха меняет СТРУКТУРУ обучения (что показано,
// сколько источников, сколько улик, какая форма ответа), а не только палитру.
// «Терминал» — финальная эпоха: к виду биржевого терминала игрок приходит последним.

export type EpochId = 'street' | 'cabinet' | 'desk' | 'terminal';

export interface EpochTokens {
  bg: string;
  surface: string;
  elevated: string;
  border: string;
...
export interface EpochDef {
  ...
  motto: string;
  goal: string; // чему учит эпоха — одна фраза
  tokens: EpochTokens;
...
export const EPOCH_ORDER: EpochId[] = ['street', 'cabinet', 'desk', 'terminal'];

export function epochForLevel(level: number): EpochDef {
  for (const id of EPOCH_ORDER) {
    const e = EPOCHS[id];
    if (level >= e.levels[0] && level <= e.levels[1]) return e;
  }
  return EPOCHS.terminal;
}

export function tokensToCss(t: EpochTokens): React.CSSProperties {
  return {
    ['--bg' as string]: t.bg,
